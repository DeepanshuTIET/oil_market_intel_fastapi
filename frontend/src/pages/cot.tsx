            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                ...
                <div key={chip.label} className="bg-bg-input border border-border rounded-xl p-3 text-center">
                  <div className="text-[10px] text-text-muted uppercase tracking-wider mb-1">{chip.label}</div>
          ...
          <Card title="COT Data Table">
            <div className="overflow-auto max-h-80">
              <table className="w-full text-xs">
                <thead><tr className="border-b border-border text-text-muted">{Object.keys(wide[0]).slice(0,8).map(k => <th key={k} className="py-2 px-2 text-left">{k}</th>)}</tr></thead>
                <tbody>{[...wide].reverse().slice(0,30).map((r: any, i) => (
                  <tr key={i} className="border-b border-border/50 hover:bg-bg-elevated/50">{Object.keys(r).slice(0,8).map(k => <td key={k} className="py-1.5 px-2 text-text-secondary">{typeof r[k] === 'number' ? formatNumber(r[k], 0) : String(r[k] ?? '')}</td>)}</tr>
                ))}</tbody>
              </table>
            </div>
          </Card>
